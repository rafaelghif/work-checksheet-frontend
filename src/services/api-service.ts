...
export const axiosPost = async <T>(url: string, payload: unknown, multipart: boolean = false): Promise<ApiResponseInterface<T>> => {
    try {
...
export const axiosPatch = async <T>(url: string, payload: unknown, multipart: boolean = false): Promise<ApiResponseInterface<T>> => {
    try {
